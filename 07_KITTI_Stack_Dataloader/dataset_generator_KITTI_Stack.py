...
class KITTI_dataset_generator():
    ...
    def getpose(self, group_sequence, sub_group):
        ...
        for sequence in group_sequence:
            ...
            #temp에 저장된 이전 페어와 새로 읽어온 current_temp를 hdf 파일에 저장 : [prev_image_path, current_image_path, prev_6DoF, current_6DoF]
            for seq_idx in seq_img_list:

                if idx==0:
                    #첫번째 페어 읽어서 prev_pose, prev_img_path에 저장
                    ...
                    prev_pose=[x, y, z, roll, pitch, yaw]
                    ########################################################################################################################
                    prev_img_path=str(self.image_dataset_path + '/' + sequence + '/image_2' + '/' + seq_idx).encode('utf-8') #utf-8로 encoding
                                                                                                                            #image_2 : 두번째 컬러 카메라
                    idx += 1
                    continue
                    ...
                    current_img_path=str(self.image_dataset_path + '/' + sequence + '/image_2' + '/' + seq_idx).encode('utf-8') #utf-8로 encoding
                                                                                                                                #image_2 : 두번째 컬러 카메라
                    sub_group.create_dataset(name=str(idx-1).zfill(10),
                                                data=[prev_img_path, current_img_path] + prev_pose + current_pose,
...
if __name__ == '__main__': 
    kitti = KITTI_dataset_generator(dataset_save_path='KITTI_stack_dataset_info.hdf5',
                                dataset_path='/media/doyu_now/Linux Workspace/ICSL_Project/Visual SLAM/KITTI_data_odometry_color')